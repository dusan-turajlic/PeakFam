...
function DayButton({
    ...
    const shortWeekDay = date.toLocaleDateString(USER_LOCAL_LANGUAGE, { weekday: 'short' });
    const dayAbbreviation = shortWeekDay[0].toUpperCase();
    ...
    return (
        <button
            ...
            className={`${[
                ...
                isToday && !isSelected ? 'border-yellow-400 border-2 bg-blue-500' : ''
            ].join(' ')}`}
            aria-label={`Select ${shortWeekDay}`}
        >
