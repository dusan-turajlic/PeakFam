...
export function DayButton({ day, date, active = false, progress = 0, className, onClick, ...props }: Readonly<DayButtonProps>) {
  ...
  const size = 64 // slightly larger than button (64px) to have space for ring
  const strokeWidth = 3
  ...
  return (
    <div className="relative inline-flex items-center justify-center">
      ...
      <button
        className={cn(
          "flex flex-col items-center justify-center gap-0 rounded-full w-16 h-16 transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold relative overflow-hidden z-0",
      ...
      >
        <RippleContainer />
        <span className="text-[10px] font-medium uppercase leading-tight">{day}</span>
